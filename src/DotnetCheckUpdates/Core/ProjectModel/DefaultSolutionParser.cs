...
internal class DefaultSolutionParser : ISolutionParser
{
    ...
    internal static IEnumerable<string> ParseProjectPathsFromSlnFile(string solutionPath)
    {
        ...
        var solutionModel = AsyncHelper.RunSync(async () => await serializer.OpenAsync(solutionPath, CancellationToken.None));

        return solutionModel.SolutionProjects
            .Select(it => Path.GetFullPath(Path.Combine(solutionRootDirectory, it.FilePath)))
            .ToArray();
    }
}

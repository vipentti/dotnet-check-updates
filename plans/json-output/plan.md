...
In JSON mode, each canonical full-path identity appears once in `checkedFiles`. Use ordinal comparison on every platform and process each resulting unique identity once. Case-variant canonical paths remain distinct, including inside case-sensitive Windows directories; aliases on case-insensitive filesystems may therefore be processed more than once rather than risking skipped files. For every exact identity group, use its canonical full-path string as the representative for `checkedFiles[].path` and every corresponding `solutions[].projects` entry, then remove repeated representative entries within each solution before sorting. This guarantees exact path joins between solution membership and checked files. Identity does not resolve symlinks, junctions, hard links, or case aliases.
...
Use one narrow pre-parse classifier for process routing, without reproducing Spectre option, grouping, help, or parse-order semantics. First derive `showStackTrace` from raw arguments and remove internal `--show-stack-trace` arguments exactly as the current application does. Run every classification rule below on the resulting `cmdArgs`, which are the same arguments passed to Spectre:

1. If first `cmdArgs` value equals `--help-dump-opencli` using `StringComparison.OrdinalIgnoreCase`, select OpenCLI mode immediately. Preserve framework OpenCLI output and status; ignore later `--json` for routing because Spectre replaces the argument list for this shortcut.
2. Otherwise, inspect only `cmdArgs` before the first exact `--` end-of-options marker. Tokens at or after `--` never establish JSON intent.
...
- Process routing classifies the same filtered `cmdArgs` passed to Spectre, recognizes the OpenCLI shortcut case-insensitively, recognizes only bare pre-boundary `--json`, rejects its explicit boolean-value forms, and leaves help, grouped options, option values, and parse order to Spectre.

...
public class CheckUpdateCommandJsonTests
{
    ...
    public void JsonModeClassifier_SelectsBareJsonBeforeBoundary()
    {
        ...
        JsonModeClassifier.Classify(["--cwd", "foo", "--json"]).Mode.Should().Be(JsonMode.JsonIntent);
    }
    ...
    public void JsonModeClassifier_IgnoresTokensAfterBoundary()
    {
        ...
        JsonModeClassifier.Classify(["--json=true", "--", "--json"]).Mode.Should().Be(JsonMode.Reject);
    ...
    {
        JsonModeClassifier.Classify(["--help-dump-opencli", "--json"]).Mode.Should().Be(JsonMode.OpenCli);
        JsonModeClassifier.Classify(["--HELP-DUMP-OPENCLI", "--json"]).Mode.Should().Be(JsonMode.OpenCli);
        JsonModeClassifier.Classify(["--Help-Dump-OpenCli", "--json"]).Mode.Should().Be(JsonMode.OpenCli);
    }
    ...
    {
        JsonPathHelper.GetKind("/some/Directory.Build.props").Should().Be(JsonOutputKind.DirectoryBuildProps);
        JsonPathHelper.GetKind("/some/Directory.Packages.props").Should().Be(JsonOutputKind.DirectoryPackagesProps);
    ...
    public void JsonPathHelper_ToJsonDisplayPath()
    {
        ...
        JsonPathHelper.ToJsonDisplayPath(file, cwd, showAbsolute: false).Should().Be(Path.Combine("src", "App", "App.csproj"));
    ...
    {
        var rawJson = await CaptureJsonAsync([("Flurl", "3.0.0"), ("Other", "1.0.0")], [new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }], new CheckUpdateCommand.Settings { Cwd = RootedTestPath("cap").ToString(), Json = true });
    ...
    {
        var rawJson = await CaptureJsonAsync([("Flurl", "3.0.0"), ("Other", "1.0.0"), ("Versionless", "")], [new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }], new CheckUpdateCommand.Settings { Cwd = RootedTestPath("cap").ToString(), Json = true, List = true });
        var doc = JsonDocument.Parse(rawJson);
        var pkgs = doc.RootElement.GetProperty("checkedFiles")[0].GetProperty("packages");
        pkgs.GetArrayLength().Should().Be(3);
        var versionless = pkgs.EnumerateArray().First(p => p.GetProperty("name").GetString() == "Versionless");
    ...
    {
        var rawJson = await CaptureJsonAsync([("Flurl", "3.0.0"), ("Other", "1.0.0")], [new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }], new CheckUpdateCommand.Settings { Cwd = RootedTestPath("cap").ToString(), Json = true, Include = ["Flurl*"] });
        var doc = JsonDocument.Parse(rawJson);
        doc.RootElement.GetProperty("checkedFiles")[0].GetProperty("packageCount").GetInt32().Should().Be(1);
    }
    ...
    public async Task Json_ShowsAbsolutePaths()
    {
        ...
        var rawJson = await CaptureProjectJsonAsync(cwd, showAbsolute: true, upgrades: [new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var doc = JsonDocument.Parse(rawJson);
    ...
    public async Task Json_UpgradeRequestedAndApplied()
    {
        ...
        var fs = SetupFileSystem(cwd.ToString(), new() { [cwd.PathCombine("a.csproj")] = ProjectFileUtils.ProjectFileXml([("Flurl", "3.0.0")]) });
        var service = SetupMockPackages([new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json1 = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, Upgrade = false });
        JsonDocument.Parse(json1).RootElement.GetProperty("upgradeRequested").GetBoolean().Should().BeFalse();
        JsonDocument.Parse(json1).RootElement.GetProperty("upgradesApplied").GetBoolean().Should().BeFalse();
        var json2 = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, Upgrade = true });
        JsonDocument.Parse(json2).RootElement.GetProperty("upgradeRequested").GetBoolean().Should().BeTrue();
        JsonDocument.Parse(json2).RootElement.GetProperty("upgradesApplied").GetBoolean().Should().BeTrue();
    }
    ...
    public async Task Json_UpgradeWithNoApplicableUpgrades()
    {
        var cwd = RootedTestPath("p");
        var fs = SetupFileSystem(cwd.ToString(), new() { [cwd.PathCombine("a.csproj")] = ProjectFileUtils.ProjectFileXml([("Flurl", "3.0.0")]) });
        var service = SetupMockPackages([new MockUpgrade("Flurl") { Versions = { "3.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, Upgrade = true });
    ...
    public async Task Json_OverlappingSolutionsDeduplicatesCheckedFiles()
    {
        ...
        var fs = SetupFileSystem(cwd.ToString(), new() { [slnA] = slnContent, [slnB] = slnContent, [proj] = ProjectFileUtils.ProjectFileXml([("Flurl", "3.0.0")]) });
        var service = SetupMockPackages([new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true });
        var doc = JsonDocument.Parse(json);
        ...
        var projPath = doc.RootElement.GetProperty("checkedFiles")[0].GetProperty("path").GetString()!;
    ...
    public async Task Json_ReportsKindsAndTargetFrameworks()
    {
        ...
        var fs = SetupFileSystem(cwd.ToString(), new() { [proj] = ProjectFileUtils.ProjectFileXml([("Flurl", "3.0.0")], framework: Frameworks.Net8_0), [props] = ProjectFileUtils.ProjectFileXml([("Central", "1.0.0")], framework: Frameworks.Net8_0, referenceType: ReferenceType.PackageVersion) });
        var service = SetupMockPackages([new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }, new MockUpgrade("Central") { Versions = { "2.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, List = true });
        var doc = JsonDocument.Parse(json);
        var byPath = doc.RootElement.GetProperty("checkedFiles").EnumerateArray().ToDictionary(e => e.GetProperty("path").GetString()!, e => e);
        byPath.Should().ContainKey("Directory.Packages.props");
        byPath["Directory.Packages.props"].GetProperty("kind").GetString().Should().Be("directoryPackagesProps");
        byPath["app.csproj"].GetProperty("kind").GetString().Should().Be("project");
        byPath["app.csproj"].GetProperty("targetFrameworks").EnumerateArray().First().GetString().Should().Be("net8.0");
    }
    ...
    public async Task Json_DeterministicOrdering()
    {
        ...
        var fs = SetupFileSystem(cwd.ToString(), new() { [a] = ProjectFileUtils.ProjectFileXml([("ZPkg", "1.0.0"), ("APkg", "1.0.0")]), [b] = ProjectFileUtils.ProjectFileXml([("MPkg", "1.0.0")]) });
        var service = SetupMockPackages([]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, List = true });
        var doc = JsonDocument.Parse(json);
        var files = doc.RootElement.GetProperty("checkedFiles").EnumerateArray().Select(e => e.GetProperty("path").GetString()).ToArray();
        files.Should().BeInAscendingOrder(StringComparer.Ordinal);
        var pkgs = doc.RootElement.GetProperty("checkedFiles").EnumerateArray().First(e => e.GetProperty("path").GetString() == "a.csproj").GetProperty("packages").EnumerateArray().Select(e => e.GetProperty("name").GetString()).ToArray();
    ...
    public async Task Json_EmptyArraysAndNulls()
    {
        ...
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, SetupMockPackages([]), finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true });
        var doc = JsonDocument.Parse(json);
    ...
    public async Task Json_VersionRangesAndInferredFrameworks()
    {
        ...
        var fs = SetupFileSystem(
            ...
            new()
            {
                [a] = ProjectFileUtils.ProjectFileXml([("RangePkg", "[1.0.0,)")], framework: Frameworks.Net8_0),
                [b] = ProjectFileUtils.ProjectFileXml([("NoFrameworkPkg", "1.0.0")], framework: Frameworks.Unspecified),
            }
        );
        var service = SetupMockPackages([new MockUpgrade("RangePkg") { Versions = { "2.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, List = true });
        var doc = JsonDocument.Parse(json);
        var byPath = doc.RootElement.GetProperty("checkedFiles").EnumerateArray().ToDictionary(e => e.GetProperty("path").GetString()!, e => e);
        byPath["a.csproj"].GetProperty("packages").EnumerateArray().First().GetProperty("currentVersion").GetString().Should().Be("[1.0.0,)");
        var targetVer = byPath["a.csproj"].GetProperty("packages").EnumerateArray().First().GetProperty("targetVersion").GetString()!;
        targetVer.Should().Be("[2.0.0,)");
        byPath["b.csproj"].GetProperty("targetFrameworks").EnumerateArray().First().GetString().Should().Be("net8.0");
    }
    ...
    public async Task Json_ConditionedDuplicatesRemainSeparate()
    {
        ...
        service.GetPackageVersionsAsync("Example", Arg.Any<CancellationToken>()).Returns(Task.FromResult(new[] { "2.0.0".ToNuGetVersion(), "3.0.0".ToNuGetVersion() }.AsEnumerable()));
        service.GetSupportedFrameworksAsync("Example", "2.0.0", Arg.Any<CancellationToken>()).Returns(Task.FromResult(ImmutableHashSet.Create(NuGet.Frameworks.FrameworkConstants.CommonFrameworks.Net80)));
        service.GetSupportedFrameworksAsync("Example", "3.0.0", Arg.Any<CancellationToken>()).Returns(Task.FromResult(ImmutableHashSet.Create(NuGet.Frameworks.FrameworkConstants.CommonFrameworks.Net90)));
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, List = true });
        var doc = JsonDocument.Parse(json);
        var pkgs = doc.RootElement.GetProperty("checkedFiles").EnumerateArray().First(e => e.GetProperty("path").GetString() == "Directory.Packages.props").GetProperty("packages");
        pkgs.GetArrayLength().Should().Be(2);
        pkgs.EnumerateArray().Select(p => p.GetProperty("currentVersion").GetString()).Should().Contain("1.0.0");
        pkgs.EnumerateArray().Select(p => p.GetProperty("currentVersion").GetString()).Should().Contain("1.5.0");
    }
    ...
    public async Task Json_ShowPackageCountInert()
    {
        ...
        var a = cwd.PathCombine("a.csproj");
        var fs = SetupFileSystem(cwd.ToString(), new() { [a] = ProjectFileUtils.ProjectFileXml([("Flurl", "3.0.0"), ("Other", "1.0.0")]) });
        var service = SetupMockPackages([new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var json = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, ShowPackageCount = true });
        var doc = JsonDocument.Parse(json);
        doc.RootElement.GetProperty("checkedFiles")[0].GetProperty("packageCount").GetInt32().Should().Be(2);
    }
    ...
    public async Task Json_FiltersPreserveSemantics()
    {
        ...
        var a = cwd.PathCombine("a.csproj");
        var fs = SetupFileSystem(cwd.ToString(), new() { [a] = ProjectFileUtils.ProjectFileXml([("System.Text.Json", "1.0.0"), ("Flurl", "3.0.0")]) });
        var service = SetupMockPackages([new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var jsonInclude = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, Include = ["Flurl*"], List = true });
        JsonDocument.Parse(jsonInclude).RootElement.GetProperty("checkedFiles")[0].GetProperty("packages").EnumerateArray().All(p => p.GetProperty("name").GetString()!.StartsWith("Flurl", StringComparison.OrdinalIgnoreCase)).Should().BeTrue();
        var jsonExclude = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, Exclude = ["Flurl*"], List = true });
        JsonDocument.Parse(jsonExclude).RootElement.GetProperty("checkedFiles")[0].GetProperty("packages").EnumerateArray().All(p => !p.GetProperty("name").GetString()!.StartsWith("Flurl", StringComparison.OrdinalIgnoreCase)).Should().BeTrue();
    }
    ...
    public async Task Json_RelativeAndAbsolutePaths()
    {
        ...
        var a = cwd.PathCombine("a.csproj");
        var fs = SetupFileSystem(cwd.ToString(), new() { [a] = ProjectFileUtils.ProjectFileXml([("Flurl", "3.0.0")]) });
        var service = SetupMockPackages([new MockUpgrade("Flurl") { Versions = { "4.0.0" }, SupportedFrameworks = MockUpgrade.DefaultSupportedFrameworks }]);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        var rel = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true });
        JsonDocument.Parse(rel).RootElement.GetProperty("checkedFiles")[0].GetProperty("path").GetString().Should().Be("a.csproj");
        var abs = await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, ShowAbsolute = true });
        JsonDocument.Parse(abs).RootElement.GetProperty("checkedFiles")[0].GetProperty("path").GetString().Should().Be(a);
    }
    ...
    public async Task Json_TextModeUnchanged()
    {
        ...
        var cmd = CheckUpdateCommandUtils.CreateCommand(console, fs, service, finder: null, SolutionFileFormat.Sln);
        await cmd.AsICommand().ExecuteAsync(new Spectre.Console.Cli.CommandContext([], Substitute.For<Spectre.Console.Cli.IRemainingArguments>(), "test", null), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), ShowAbsolute = true, AsciiTree = true }, CancellationToken.None);
    ...
    private static async Task<string> CaptureProjectJsonAsync(FullPath cwd, bool showAbsolute, IEnumerable<MockUpgrade> upgrades)
    {
        var fs = SetupFileSystem(cwd.ToString(), new() { [cwd.PathCombine("a.csproj")] = ProjectFileUtils.ProjectFileXml([("Flurl", "3.0.0")]) });
        var service = SetupMockPackages(upgrades);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
        return await RunJsonAsync(cmd, cwd.ToString(), new CheckUpdateCommand.Settings { Cwd = cwd.ToString(), Json = true, ShowAbsolute = showAbsolute });
    }

    private static async Task<string> CaptureJsonAsync(IEnumerable<(string id, string version)> packages, IEnumerable<MockUpgrade> upgrades, CheckUpdateCommand.Settings settings)
    {
        var cwd = RootedTestPath("cap");
        var fs = SetupFileSystem(cwd.ToString(), new() { [cwd.PathCombine("a.csproj")] = ProjectFileUtils.ProjectFileXml(packages) });
        var service = SetupMockPackages(upgrades);
        var cmd = CheckUpdateCommandUtils.CreateCommand(new TestConsole(), fs, service, finder: null, SolutionFileFormat.Sln);
    ...
    private static async Task<string> RunJsonAsync(CheckUpdateCommand cmd, string cwd, CheckUpdateCommand.Settings settings)
    {
        ...
        try
        {
            await cmd.AsICommand().ExecuteAsync(new Spectre.Console.Cli.CommandContext([], Substitute.For<Spectre.Console.Cli.IRemainingArguments>(), "test", null), settings, CancellationToken.None);
        }
        finally
        {
            Console.SetOut(prev);
